...
function parseVideoTitles(data) {
    const titles = {
        ...
        fr: String(data.titles?.fr ?? data.title_fr ?? '').trim(),
        ka: String(data.titles?.ka ?? data.title_ka ?? '').trim(),
    };
    if (!titles.tr && !titles.de && !titles.en && !titles.fr && !titles.ka) return null;
    if (!titles.tr) titles.tr = titles.de || titles.en || titles.fr || titles.ka;
    if (!titles.de) titles.de = titles.tr;
    if (!titles.en) titles.en = titles.tr;
    if (!titles.fr) titles.fr = titles.tr;
...
function normalizeVideoDoc(video) {
    ...
    return {
        ...video,
        titles: { tr: legacy, de: legacy, en: legacy, fr: legacy, ka: legacy },
    };
}
...
function parseCarouselCtaLabels(data) {
    ...
    if (!raw.tr && !raw.de && !raw.en && !raw.fr && !raw.ka) return { tr: '', de: '', en: '', fr: '', ka: '' };
    return fillCarouselLangs(raw);
}
...
function validateCarouselPayload(data) {
    ...
    if (cta_link && !cta_labels.tr && !cta_labels.de && !cta_labels.en && !cta_labels.fr && !cta_labels.ka) {
...
const DEFAULT_CAROUSEL_COUNT = 14;
