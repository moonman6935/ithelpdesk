...
function CarouselSlide({ slide, index, slidesLength, isActive, t, direction, slideKey, isWelcomeSlide }) {
  ...
  return (
    <div className={`min-w-full bg-gradient-to-br ${gradient} text-white relative overflow-hidden`}>
      ...
      <div
        key={slideKey}
        className={`relative grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-10 items-center p-5 sm:p-10 md:p-16 min-h-[240px] sm:min-h-[400px] md:min-h-[460px] lg:min-h-[500px] home-hero-slide ${isActive ? enterClass : ''}`}
      >
        <div className="order-2 md:order-1 space-y-3 sm:space-y-5">
          <span className="inline-block px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full bg-white/20 text-xs sm:text-sm font-medium sm:backdrop-blur-sm border border-white/20">
          ...
          <p
            className={`text-sm sm:text-xl md:text-2xl text-white/95 leading-relaxed max-w-2xl lg:max-w-3xl line-clamp-2 sm:line-clamp-none ${
        ...
        <div className="order-1 md:order-2 flex justify-center">
          <div className={`relative ${isActive ? 'ft-icon-burst' : ''}`}>
            {isActive && <span className="ft-energy-ring rounded-full hidden sm:block" />}
            <div className="w-28 h-28 sm:w-52 sm:h-52 md:w-64 md:h-64 lg:w-72 lg:h-72 rounded-full bg-white/15 sm:backdrop-blur-sm flex items-center justify-center border-2 sm:border-4 border-white/30 shadow-inner relative">
              <div
                className="absolute inset-0 rounded-full opacity-40 animate-spin carousel-icon-spin hidden sm:block"
                style={{
                  background:
                    'conic-gradient(from 0deg, transparent, rgba(0,240,255,0.4), transparent, rgba(255,0,170,0.3), transparent)',
                }}
              />
              <Icon className="w-14 h-14 sm:w-28 sm:h-28 md:w-32 md:h-32 lg:w-36 lg:h-36 text-white drop-shadow-lg relative z-10" strokeWidth={1.25} />
            </div>
      ...
      <div className="site-container">
        <div className="w-full relative">
          ...
          <button
            type="button"
            onClick={prev}
            className="absolute left-2 sm:left-0 top-1/2 -translate-y-1/2 sm:-translate-x-1 md:-translate-x-5 z-10 w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-white/95 shadow-lg border flex items-center justify-center text-gray-700 hover:bg-cyan-50 hover:text-cyan-600 hover:shadow-cyan-200/50 hover:scale-110 transition-all duration-300"
            aria-label="Önceki"
          >
          ...
          <button
            type="button"
            onClick={next}
            className="absolute right-2 sm:right-0 top-1/2 -translate-y-1/2 sm:translate-x-1 md:translate-x-5 z-10 w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-white/95 shadow-lg border flex items-center justify-center text-gray-700 hover:bg-fuchsia-50 hover:text-fuchsia-600 hover:shadow-fuchsia-200/50 hover:scale-110 transition-all duration-300"
            aria-label="Sonraki"
          >
            <ChevronRight className="w-4 h-4 sm:w-5 sm:h-5" />
          </button>

          <div className="flex justify-center gap-1.5 sm:gap-2 mt-3 sm:mt-5 flex-wrap px-2 max-w-full overflow-hidden">
            {slides.map((slide, index) => (
              <button
                ...
                className={`h-2 sm:h-2.5 rounded-full transition-all duration-500 ${
                  index === active
                    ? 'w-6 sm:w-8 bg-gradient-to-r from-cyan-500 via-violet-500 to-fuchsia-500 ft-dot-active'
                    : 'w-2 sm:w-2.5 bg-gray-300 hover:bg-violet-300 hover:scale-125'
                }`}
