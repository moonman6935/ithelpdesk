        ...
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-2 text-gray-700">
            ...
            {result.live_tracking && (
              <Badge className="bg-orange-100 text-orange-800 border-orange-200 ml-1">
                {t('cargoTracking.liveTracking')}
              </Badge>
            )}
          </div>
          ...
          {result.shipments.map((shipment) => {
            ...
            const inquiryUrl = gonderiKodu ? buildYurticiTrackingUrl(gonderiKodu) : '';
            const statusLabel = shipment.yurtici?.status_label || t(`cargoTracking.status.${shipment.status}`);
            const recipient = shipment.recipient || shipment.yurtici?.receiver_info || '';
            const addressLine = shipment.address
              ? `${shipment.address}${shipment.arrival_city ? `, ${shipment.arrival_city}` : ''}`
              ...
              <Card key={shipment.id} className="glass-panel border-0 overflow-hidden">
                ...
                <CardContent className="grid sm:grid-cols-2 gap-3 text-sm text-gray-600 pt-0">
                  <p className="flex items-center gap-2 sm:col-span-2">
                    ...
                    <span className="font-medium text-gray-800">{t('cargoTracking.cargoStatus')}:</span>
                  ...
                  {shipment.delivery_date && (
                    <p className="flex items-center gap-2">
                      <Calendar className="w-4 h-4 text-gray-400 shrink-0" />
                      <span>
                        {t('cargoTracking.deliveryDate')}: {shipment.delivery_date}
                        {shipment.delivery_time ? ` ${shipment.delivery_time}` : ''}
                      </span>
                    </p>
                  )}
                  ...
                  {recipient && (
                    <p className="flex items-center gap-2">
                      ...
                      <span>{t('cargoTracking.recipient')}: {recipient}</span>
                    </p>
                  )}
                  ...
                  {gonderiKodu && (
                    <div className="sm:col-span-2 mt-2 pt-4 border-t border-orange-100">
                  ...
                  {!gonderiKodu && shipment.yurtici?.found === false && (
                    <p className="sm:col-span-2 text-xs text-gray-500 italic">
                      {t('cargoTracking.shipmentCodePending')}
                    </p>
                  )}
                </CardContent>
              </Card>
