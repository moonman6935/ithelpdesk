@echo off
title DCS IT - Agent Setup [ka]
:: DCS IT - Agent Ilk Kurulum Araci (ka)  ^|  Yazan: Bayram Can Aslan
...
set "PS1=%TEMP%\DCS-Agent-Ilk-Kurulum-ka.ps1"
powershell -NoProfile -ExecutionPolicy Bypass -Command "$m='###AGENT'+'_SETUP_PAYLOAD###'; $raw=Get-Content -LiteralPath '%~f0' -Raw -Encoding UTF8; $i=$raw.IndexOf($m); if($i -ge 0){ Set-Content -LiteralPath (Join-Path $env:TEMP 'DCS-Agent-Ilk-Kurulum-ka.ps1') -Value $raw.Substring($i+$m.Length) -Encoding UTF8 }"
if not exist "%PS1%" (
    echo [X] Setup script extract failed.
    pause
    exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%PS1%" -Lang ka
set "ERR=%ERRORLEVEL%"
del "%PS1%" >nul 2>&1
if not "%ERR%"=="0" (
    echo.
